...
void ofxSurfingMixer::setupParamsInternal() {
	...
	params_UserGui.add(bEnableChannel1);
	params_UserGui.add(bEnableChannel2);
...
void ofxSurfingMixer::setupParamsMixer() {
	...
	//params_Blend.add(bEnableModeBlend);
	...
	params_Control.add(bEnableModeBlend);
	params_Control.add(bEnableModeMixer);
...
void ofxSurfingMixer::end_Channel_2() {
	...
	if (bEnableModeMixer) {
		if (!bSwapChannels) {
			if (bEnableChannel2) fbo_Mixer_B.end();
		} else {
			if (bEnableChannel1) fbo_Mixer_A.end();
		}
...
void ofxSurfingMixer::begin_Mix() {
	...
	if (bEnableModeBlend) {
		if (!bSwapChannels) {
			if (bEnableChannel2) psBlend.draw(fbo_Input_2.getTextureReference(), blendMode);
		} else {
			if (bEnableChannel1) psBlend.draw(fbo_Input_1.getTextureReference(), blendMode);
		}
	}
#endif
...
void ofxSurfingMixer::setupCallbacks() {
	...
	parameterListeners.push(bResetBackgrounds.newListener([this](bool & value) {
		...
		bEnableChannel1 = true;
		bEnableChannel2 = true;
		bEnableBg1 = true;
		bEnableBg2 = true;
		bEnableBgMix = true;
...
#ifdef INCLUDE_ofxGui
		positionGui = glm::vec2(gui.getPosition());
#endif
...
void ofxSurfingMixer::guiRefresh() {
	...
	if (!bEnableModeBlend && !bEnableModeMixer) {
		MODE_AppMixer = 2;
		bEnableModeMixer = true;
	}
